...
const TeamMembershipScreen = () => {
  ...
  const activeUsers = useMemo(
    () => users?.filter((user) => user.is_active && user.is_confirmed) ?? [],
    [users],
  );
  ...
  const sections = useMemo(() => {
    ...
    if (filteredInvitedUsers.length > 0) {
      arr.push({
        ...
        data: filteredInvitedUsers.sort((a, b) =>
          a.first_name.localeCompare(b.first_name),
        ),
      });
    }
    ...
    <KeyboardAvoidingView style={styles.container} behavior="padding">
      ...
      <SectionList
        ...
        keyExtractor={(item, index) =>
          item.id ??
          `${item.first_name}-${item.last_name}-${index}-${item.avatar_url}`
        }
        ListEmptyComponent={
        ...
        renderSectionHeader={({ section: { title } }) => (
          <Text style={[TextStyles.largeLabel, styles.sectionLabel]}>
            {title}
          </Text>
        )}
        ...
        renderItem={({ item }) => (
          ...
          >
            <ProfileCardThin
              ...
              locations={item.memberships.map(
                (membership) => membership.location_id,
              )}
              role={
                RoleLabels[
                  getHighestRole(
                    item.memberships.find(
                      (membership) =>
                        membership.location_id === selectedLocation,
                    )?.roles ?? [],
                  ) ?? Role.TEAM_MEMBER
                ]
              }
